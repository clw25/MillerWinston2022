...
samples = samples %>% separate(Sample, c('genotype','replicate','condition','type'), sep = "-")
...
{if ((samples[i,4]=='input') & samples[i,5]=='Scer')
...
#IP or RNH = 1/(reads-IP-spike-in * (reads-input-experimental/reads-input-spikein))
  else if ((samples[i,4]=='IP') & samples[i,5]=='Scer')
{IP_Spom = as.numeric(as.character(samples$Reads[samples$genotype == samples$genotype[i] & samples$replicate == samples$replicate[i] & samples$condition == samples$condition[i] & samples$type == samples$type[i] & samples$species =='Spom']))
  input_Spom= as.numeric(as.character(samples$Reads[samples$genotype == samples$genotype[i] & samples$replicate == samples$replicate[i] & samples$condition == samples$condition[i] & samples$type == 'input' & samples$species =='Spom']))
  input_Scer= as.numeric(as.character(samples$Reads[samples$genotype == samples$genotype[i] & samples$replicate == samples$replicate[i] & samples$condition == samples$condition[i] & samples$type == 'input' & samples$species =='Scer']))
